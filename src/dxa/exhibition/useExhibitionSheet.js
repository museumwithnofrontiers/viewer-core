...
// What is declared here is only what is an exhibition's: the field
...
// project name follows an exhibition's own rule. Every label is an entry
...
// a language an exhibition does not offer) they fall back to English,
...
/**
 * The exhibition's item-sheet spec, over its data layer's `data`.
 *
 * Reads `data.defaultLang`, `data.itemRoute`, `data.labelOf`,
 * `data.mdInline`, `data.mdStrip`, `data.partnerById`, `data.projectName`,
 * `data.tr`, `data.translations`. `config` carries nothing today.
 */
export function useExhibitionSheet(data, config = {}) {
  void config
  const { defaultLang, itemRoute, labelOf, mdInline, mdStrip, partnerById, projectName, tr, translations } = data
  ...
  const itemSheet = {
    ...
    citation: {
      ...
      // The manifest's own project name for this record, read in the
      // record's own display language, not the site's active one — a
      // record legacy left nameless (the Explore monuments database) still
      // cites nothing.
